...
public class EmojiTextView extends TextView {
    ...
    private ImageSpan getImageSpan(String content) {
        ...
        int id = getContext().getResources().getIdentifier(EmojiUtil.STATIC_FACE_PREFIX + idStr, "drawable",
    ...
    private DynamicDrawableSpan getDynamicImageSpan(String content) {
        String idStr = EmojiUtil.getInstance().getFaceId(content);
        int id = getContext().getResources().getIdentifier(EmojiUtil.DYNAMIC_FACE_PREFIX + idStr, "drawable",
                getContext().getPackageName());
        if (id > 0) {
            try {
