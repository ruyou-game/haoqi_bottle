...
import haoqi.emoji.util.EmojiUtil;
...
public class EmojiEditText extends EditText {
    ...
    private ImageSpan getImageSpan(String content) {
        ...
        int id = resources.getIdentifier(EmojiUtil.STATIC_FACE_PREFIX + idStr, "drawable", getContext().getPackageName());
        if (id > 0) {
            try {
    ...
    private DynamicDrawableSpan getDynamicImageSpan(String content) {
        String idStr = EmojiUtil.getInstance().getFaceId(content);
        Resources resources = getContext().getResources();
        int id = resources.getIdentifier(EmojiUtil.DYNAMIC_FACE_PREFIX + idStr, "drawable", getContext().getPackageName());
        if (id > 0) {
            try {
